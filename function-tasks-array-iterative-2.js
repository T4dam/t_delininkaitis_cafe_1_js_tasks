...
);
{
  const studentAverages = students.map(({ modules, ...rest }) => ({
    ...
    avg:
      modules.reduce(
        (result, { marks, credits }) =>
          result +
          (credits * marks.reduce((acc, mark) => acc + mark)) / marks.length,
        0
      ) / modules.reduce((acc, { credits }) => acc + credits, 0),
  }));
...
{
  students
    .filter(({ faculty }) => faculty === "Informatikos fakultetas")
    .forEach((s) =>
      console.log(s.name, s.surname, s.faculty, s.course, s.modules)
    );
}
console.groupEnd();
...
{
  students
    .filter(({ faculty }) => faculty === "Chemijos fakultetas")
    .forEach((s) =>
      console.log(s.name, s.surname, s.faculty, s.course, s.modules)
    );
}
...
{
  students
    .filter(({ faculty }) => faculty === "Elektros ir elektronikos fakultetas")
    .forEach((s) =>
      console.log(s.name, s.surname, s.faculty, s.course, s.modules)
    );
}
...
{
  const ChemistryStudentsAvg = students
    .filter(({ faculty }) => faculty === "Chemijos fakultetas")
    .map(({ modules, ...rest }) => ({
      ...rest,
      modules,
      avg:
        modules.reduce(
          (result, { credits, marks }) =>
            result +
            (credits * marks.reduce((acc, mark) => acc + mark, 0)) /
              marks.length,
          0
        ) / modules.reduce((acc, { credits }) => acc + credits, 0),
    }));
  console.log(ChemistryStudentsAvg);
}
console.groupEnd();
...
{
  const ElectricStudentsAvg = students
    .filter(({ faculty }) => faculty === "Elektros ir elektronikos fakultetas")
    .map(({ modules, ...rest }) => ({
      ...rest,
      modules,
  ...
  console.log(ElectricStudentsAvg);
}
console.groupEnd();
...
console.groupCollapsed("10. Atspausdinti tik trečio kurso studentus");
{
  students
    ...
    .forEach((s) => console.log(s.name, s.surname, s.faculty, s.modules));
}
console.groupEnd();
