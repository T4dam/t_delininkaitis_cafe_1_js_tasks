...
);
{
  ...
  );
  {
    ...
    {
      console.table(people.filter((p) => p.name.length > 6));
    }
    console.groupEnd();
    console.group("kurių svoris didesnis nei 80kg");
    {
      const result = people.filter((p) => p.weight > 80);
      console.table(result);
    }
    console.groupEnd();
    console.group("kurie aukštesni nei 185cm");
    {
      const result = people.filter((p) => p.height > 185);
      console.table(result);
    }
    console.groupEnd();
  }
  ...
  );
  {
    console.group("ūgius");
    {
      const result = people.map((p) => p.height);
      console.log(result);
    }
    console.groupEnd();
    ...
    {
      console.table(
        people.map(({ height, weight, age }) => ({ height, weight, age }))
      );
    }
    console.groupEnd();
  ...
  );
  {
    ...
    {
      console.table(
        people.reduce((sum, p) => sum + p.weight, 0) / people.length
      );
    }
    console.groupEnd();
    ...
    {
      console.table(
        people.reduce((sum, p) => sum + p.height, 0) / people.length
      );
    }
    console.groupEnd();
  }
