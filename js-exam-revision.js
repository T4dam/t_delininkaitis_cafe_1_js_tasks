...
);
{
  const calcBMI = (weight, heightCM) =>
    Math.round((10 * weight) / (heightCM / 100) ** 2) / 10;
  ...
  console.groupCollapsed(
    "4. Panaudojant Array.prototype.map atrinkti į naują masyvą ir po to atspausdinti"
  );
  {
    console.group("ūgius");
    {
    ...
    {
      console.table(
        people.map(({ weight, height }) => calcBMI(weight, height))
      );
    }
    console.groupEnd();
...
);
{
  class Person {
    ...
    get string() {
      ...
      const propString = Object.entries(rest)
        ...
        .reduce(
          (propString, [prop, val]) => propString + `\n\t${prop}: ${val}`,
          ""
        );
  ...
  const peopleArray = people.map((p) => new Person(p));
  ...
  );
  {
    ...
    peopleArray.forEach((p) => console.log(p.bmi));
    peopleArray.forEach((p) => console.log(p.string));
  }
  console.groupEnd();
  ...
  {
    const result = peopleArray.filter(
      (p) => p.sex === "female" && p.age < 20 && p.weight > 70
    );
  ...
  {
    const result = peopleArray.filter(
      (p) => p.sex === "male" && p.age > 25 && p.bmi < 18.5
    );
    console.table(result);
  }
  console.groupEnd();
  ...
  {
    const ciruliai = peopleArray.filter((p) => p.age < 18 && p.bmi > 30);
    console.table(ciruliai);
  }
  console.groupEnd();

  console.group("4. Atrinkti pensininkus, su antsvoriu (KMI > 30)");
  {
    const result = peopleArray.filter((p) => p.age > 65 && p.bmi > 30);
    console.table(result);
  }
  console.groupEnd();
  ...
  {
    const result = peopleArray.filter((p) => p.bmi < 18.5 || p.bmi > 25);
    console.table(result);
  }
  console.groupEnd();
}
